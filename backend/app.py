...
# Configure Gemini AI
# Configure Gemini AI
GEMINI_API_KEY = 'AIzaSyB_kYJFzG3q0Oe06INdci1BKyRRwDDFvdk'
# Alternative: Use environment variable for security
# GEMINI_API_KEY = os.environ.get('GEMINI_API_KEY')

if not GEMINI_API_KEY:
    logger.warning("GEMINI_API_KEY not set. Chatbot features may not work.")
...
CORS(app, origins=allowed_origins, supports_credentials=True)  # Enable CORS for frontend communication
app.config['SECRET_KEY'] = 'your-secret-key-here'
app.config['UPLOAD_FOLDER'] = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'uploads')
app.config['MODEL_DIR'] = 'models'
app.config['MAX_CONTENT_LENGTH'] = 16 * 1024 * 1024  # 16MB max file size
app.config['SESSION_COOKIE_SECURE'] = os.environ.get('FLASK_ENV') == 'production' # True in production, False in dev
...
def load_stage1_model() -> Optional[nn.Module]:
    ...
    model_path = os.path.join(app.config['MODEL_DIR'], 'fundus_classifier.pth')
    
    # Check if model exists, if not, try to download it
    if not os.path.exists(model_path):
        logger.warning(f"Stage 1 model file not found at {model_path}.")
        # Try to download the model
        try:
            import gdown
            url = 'https://drive.google.com/uc?id=1sWu9cAiz7Z2DqnEsXllpqOcMToBDF2Z7'
            logger.info("Attempting to download Stage 1 model from Google Drive...")
            gdown.download(url, model_path, quiet=False)
            logger.info("Stage 1 model downloaded successfully.")
        except Exception as e:
            logger.error(f"Failed to download Stage 1 model: {e}")
            logger.warning("Using rule-based validation as fallback.")
            return None
    ...
    try:
        ...
        logger.info(f"Stage 1 FundusClassifier model loaded successfully from {model_path}")
        return model
...
def load_clinical_model() -> Optional[nn.Module]:
    ...
    # Use relative path instead of absolute path
    model_path = os.path.join(app.config['MODEL_DIR'], 'dr_model.pth')
    
    # Check if model exists, if not, try to download it
    if not os.path.exists(model_path):
        logger.warning(f"Clinical model file not found at {model_path}.")
        # Try to download the model
        try:
            import gdown
            url = 'https://drive.google.com/uc?id=18tJ7d4BbdWReVUCWSvfF_cY3yMh46PZI'
            logger.info("Attempting to download Clinical model from Google Drive...")
            gdown.download(url, model_path, quiet=False)
            logger.info("Clinical model downloaded successfully.")
        except Exception as e:
            logger.error(f"Failed to download Clinical model: {e}")
            return None
    ...
    try:
        ...
        logger.info(f"Clinical model loaded successfully from {model_path}")
        return model
...
def load_all_models() -> Dict[str, Tuple[Any, Any, Optional[str]]]:
    ...
    model_paths = {
        ...
        'densenet121': os.path.join(app.config['MODEL_DIR'], 'model4.pth')
    }

    # Check if model files exist, if not, try to download them
    model_urls = {
        'resnet50': 'https://drive.google.com/uc?id=1gFACSGC_7PON0xZoII6PJ5I9duk9ePGs',
        'efficientnet': 'https://drive.google.com/uc?id=12codw5Zjq-ShLuhd_CUE_ZWf8DKUsRd0',
        'vgg16': 'https://drive.google.com/uc?id=1MhQVHXSKMOTWItuau-Idjn4aSNQhACQJ',
        'densenet121': 'https://drive.google.com/uc?id=1TPTcVIZx0NPmHypNuoIMCop8kkgxQgZZ'
    }
    
    for arch, model_path in model_paths.items():
        if not os.path.exists(model_path):
            logger.warning(f"Model file for {arch} not found at {model_path}.")
            # Try to download the model
            try:
                import gdown
                url = model_urls[arch]
                logger.info(f"Attempting to download {arch} model from Google Drive...")
                gdown.download(url, model_path, quiet=False)
                logger.info(f"{arch} model downloaded successfully.")
            except Exception as e:
                logger.error(f"Failed to download {arch} model: {e}")
...
def register_api():
    ...
    try:
        conn = sqlite3.connect('diabetic_retinopathy.db', timeout=30)
        # Enable WAL mode for better concurrency
        conn.execute("PRAGMA journal_mode = WAL")
        c = conn.cursor()
...
def update_doctor_appointment_status_api(appointment_id: int):
    try:
        ...
        c.execute('SELECT patient_id, appointment_date, appointment_time FROM appointments WHERE id = ?', (appointment_id,))
        appt_info = c.fetchone()
        
        c.execute('UPDATE appointments SET status = ? WHERE id = ?', (new_status, appointment_id))
        
        # Create notification for patient
        if appt_info:
            patient_id, appt_date, appt_time = appt_info
            c.execute('''INSERT INTO notifications (user_id, message, type)
                        VALUES (?, ?, ?)''',
                     (patient_id, f"Your appointment on {appt_date} at {appt_time} has been {new_status}.", 'appointment_update'))
        conn.commit()
        conn.close()
...
@app.route('/api/shared-reports-history', methods=['GET'])
@role_required('patient')
def shared_reports_history_api():
    try:
        conn = sqlite3.connect('diabetic_retinopathy.db')
        conn.execute("PRAGMA journal_mode = WAL")
        c = conn.cursor()
        c.execute('''
            SELECT sr.id, r.filename, r.final_prediction, r.predictions, r.created_at, u.full_name as doctor_name, sr.shared_at, sr.status, r.id as original_report_id
            FROM shared_reports sr
            JOIN reports r ON sr.report_id = r.id
        ...
        history = []
        for r in rows:
            # logic to get string result from final_prediction
            filename = r[1]
            final_pred_idx = r[2]
            predictions_json = r[3]
            
            result_text = 'Unknown'
            if filename == 'clinical_data':
                 try:
                     preds = json.loads(predictions_json)
                     result_text = preds.get('prediction_text', 'Clinical Analysis')
                 except:
                     result_text = 'Clinical Analysis'
            else:
                 if isinstance(final_pred_idx, int) and final_pred_idx in DR_STAGES:
                     ...
                     result_text = str(final_pred_idx)
            ...
            history.append({
                ...
                'shared_at': r[6],
                'status': r[7],
                'original_report_id': r[8]
            })
            
        conn.close()
        return jsonify(history), 200
    except Exception as e:
        logger.error(f"Error fetching shared reports history: {e}")
        return jsonify({'success': False, 'message': 'Error loading shared reports history.'}), 500
...
def appointments_api():
    if request.method == 'GET':
        ...
        try:
            ...
            c.execute('SELECT full_name FROM users WHERE id = ?', (session['user_id'],))
            patient_name_row = c.fetchone()
            patient_name = patient_name_row[0] if patient_name_row else "A patient"
            
            c.execute('''INSERT INTO notifications (user_id, message, type)
                        VALUES (?, ?, ?)''',
                     (doctor_id, f"New appointment request from {patient_name} for {appointment_date} at {appointment_time}", 'appointment_request'))
            conn.commit()
            conn.close()
...
def update_shared_report_status_api(shared_report_id):
    try:
        ...
        # Notify patient when report status changes to 'reviewed'
        if new_status == 'reviewed':
            ...
            row = c.fetchone()
            if row:
                patient_id, report_id = row
                c.execute('''INSERT INTO notifications (user_id, message, type)
                            VALUES (?, ?, ?)''', 
                            (patient_id, f"Your report #{report_id} has been reviewed by the doctor.", 'report_reviewed'))
                conn.commit()
...
def share_report_api():
    ...
    try:
        ...
        # PROACTIVE: Create notification for the doctor
        # We need the patient's name
        c.execute('SELECT full_name FROM users WHERE id = ?', (session['user_id'],))
        ...
        c.execute('''INSERT INTO notifications (user_id, message, type)
                     VALUES (?, ?, ?)''', 
                     (doctor_id, f"New report shared by {patient_name} (Report #{report_id})", 'report_shared'))
...
def get_notifications_api():
    try:
        ...
        conn.execute("PRAGMA journal_mode = WAL")
        c = conn.cursor()
        c.execute('SELECT id, message, type, is_read, created_at FROM notifications WHERE user_id = ? ORDER BY created_at DESC', (session['user_id'],))
        notifications_db = c.fetchall()
...
def mark_notifications_read_api():
    try:
        ...
        c.execute('UPDATE notifications SET is_read = 1 WHERE user_id = ? AND is_read = 0', (session['user_id'],))
        conn.commit()
        conn.close()
