...
import { Eye, ArrowRight, Image as ImageIcon, AlertCircle, X as XIcon, ArrowLeft } from 'lucide-react';
    ...
    <div className="min-h-screen bg-background-100">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-8 relative">
          <Link
            to="/new-analysis"
            className="absolute left-0 top-0 p-2 text-primary-600 hover:text-primary-800 hover:bg-primary-100 rounded-lg transition-colors lg:translate-y-2"
              ...
              <div className="space-y-4">
                {[
                  ...
                  <div key={item.step} className={`flex items-start space-x-3 ${item.active ? 'opacity-100' : 'opacity-60'}`}>
                    <div className={`rounded-full w-6 h-6 flex items-center justify-center text-xs font-bold ${item.active ? 'bg-primary-600 text-white' : 'bg-gray-200 text-gray-600'
                      }`}>
                      {item.step}
                    </div>
                    <div>
