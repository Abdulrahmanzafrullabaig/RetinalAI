...
const ResultDetail = () => {
  ...
  const handleDownload = async () => {
    if (!result) return;
    try {
      const response = await fetch(`${API_URL}/api/download-report/${result.id}`, {
        method: 'GET',
        credentials: 'include',
      });
      if (response.ok) {
  ...
  const handleShare = async () => {
    ...
    setShareError('');
    try {
      const response = await fetch(`${API_URL}/api/share-report`, {
        method: 'POST',
            ...
            <div className="p-8 overflow-y-auto max-h-[calc(85vh-140px)] font-body">
                    ...
                    if (cleanLine.startsWith('WHY THIS PREDICTION') ||
                      ...
                      return (
                        <div key={index} className="flex items-start space-x-3 mb-4 ml-6">
                          ...
                          <p className="text-gray-900 font-body leading-relaxed text-lg">{cleanLine.substring(1).trim()}</p>
                        </div>
                      );
                    } else if (cleanLine.trim()) {
                      return (
                        <div key={index} className="mb-6">
                          <p className="text-gray-900 font-body leading-relaxed text-lg">
                            {cleanLine}
                          </p>
                        </div>
                      );
              ...
              <div className="mt-8 pt-6 border-t border-gray-200">
                <div className="flex flex-col lg:flex-row items-center justify-between space-y-4 lg:space-y-0 lg:space-x-6">
                  ...
                  <button
                    onClick={() => setShowInsights(false)}
                    className="text-gray-600 hover:text-gray-900 px-6 py-3 rounded-lg transition-colors font-heading font-medium"
                  >
                    Close Insights
                  </button>
                </div>
