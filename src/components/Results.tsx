  ...
  const fetchResults = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setError(errorData.message || 'Failed to loaded results');
      }
    } catch (err) {
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="space-y-4">
          {results.length > 0 ? (
            ...
            <div className="text-center py-8 text-primary-600">
              No analysis results found. <Link to="/predict" className="text-secondary-500 hover:underline">Start a new analysis</Link>!
            </div>
          )}
        </div>
        ...
        <div className="mt-8 grid lg:grid-cols-3 gap-6">
          <div className="bg-white rounded-xl p-6 shadow-sm border border-primary-100">
            ...
            <p className="text-3xl font-bold text-secondary-500">{results.length}</p>
