...
const DoctorDashboard = () => {
  ...
  const fetchSharedReports = async () => {
    ...
    setError('');
    try {
      const response = await fetch(`${API_URL}/api/shared-reports`, {
        method: 'GET',
        credentials: 'include',
      });
      if (response.ok) {
  ...
  const updateAppointmentStatus = async (appointmentId: number, status: 'accepted' | 'declined' | 'pending') => {
    try {
      const response = await fetch(`${API_URL}/api/doctor-appointments/${appointmentId}/status`, {
        method: 'POST',
  ...
  const handleReview = async (sharedReportId: number, currentStatus: string) => {
    ...
    try {
      const response = await fetch(`${API_URL}/api/shared-reports/${sharedReportId}/status`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
  ...
  const handleDownload = async (reportId: number) => {
    try {
      ...
      const response = await fetch(`${API_URL}/api/download-report/${reportId}`, {
        method: 'GET',
        credentials: 'include',
      });
      if (response.ok) {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid lg:grid-cols-4 gap-6 mb-8">
          <div className="bg-white rounded-xl p-6 shadow-sm border border-primary-100">
            ...
            <h3 className="text-2xl font-bold text-gray-900">{totalSharedReports}</h3>
          ...
          <div className="bg-white rounded-xl p-6 shadow-sm border primary-100">
            ...
            <h3 className="text-2xl font-bold text-gray-900">{upcomingAppointments}</h3>
        ...
        <div className="mb-6">
          <div className="flex space-x-4 border-b border-primary-100">
            <button
              ...
              className={`px-4 py-2 font-medium text-sm transition-colors ${activeTab === 'shared-reports'
            ...
            <button
              ...
              className={`px-4 py-2 font-medium text-sm transition-colors ${activeTab === 'patient-management'
                  ? 'text-primary-700 border-b-2 border-primary-600'
                  : 'text-primary-600 hover:text-primary-700'
                }`}
            >
            ...
            <button
              ...
              className={`px-4 py-2 font-medium text-sm transition-colors ${activeTab === 'appointments'
                  ? 'text-primary-700 border-b-2 border-primary-600'
                  : 'text-primary-600 hover:text-primary-700'
                }`}
            >
                  ...
                  filteredReports.map((report) => (
                    <div key={report.id} className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-4 bg-primary-50 rounded-lg hover:bg-primary-100 transition-colors">
                      <div className="flex items-start space-x-4 mb-3 sm:mb-0">
                        ...
                        <div>
                          <h3 className="font-semibold text-gray-900">{report.patient_name}</h3>
                          <p className="text-sm text-gray-500">{report.patient_email}</p>
                    ...
                    <div key={appt.id} className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-4 bg-primary-50 rounded-lg">
                      <div className="flex items-start space-x-4 mb-3 sm:mb-0">
                        ...
                        <div>
                          ...
                          {appt.reason && (
                            <p className="text-xs text-gray-500 mt-1">Reason: {appt.reason}</p>
                          )}
                        </div>
                      </div>
                      <div className="flex items-center gap-3 w-full sm:w-auto">
